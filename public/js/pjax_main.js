(() => {
  ...
  var getRealPath = (pathname = window.location.pathname, desc = false) => {
    const names = pathname.split("/").filter((rawName) => {
      const n = rawName.trim();
      return n.length > 0 && n !== "/" && n !== "index.html";
    });
    if (desc) {
      return names[0] || "/";
    } else {
  ...
  _$$(
    ...
    if (window.siteConfig.icon_font) {
      element.innerHTML = window.siteConfig.anchor_icon ? `&#x${window.siteConfig.anchor_icon};` : window.siteConfig.anchor_icon === false ? "" : "&#xe635;";
    } else {
      element.innerHTML = window.siteConfig.anchor_icon ? `&#x${window.siteConfig.anchor_icon};` : window.siteConfig.anchor_icon === false ? "" : "&#xf292;";
    }
  });
  ...
  _$$(".sidebar-toc-btn").forEach((element) => {
    element.off("click").on("click", function() {
      ...
      _$$(".sidebar-toc-btn").forEach((el) => el.classList.remove("current"));
      this.classList.add("current");
      _$$(".sidebar-common-btn").forEach((el) => el.classList.remove("current"));
    ...
    const activateNavByIndex = (index) => {
      ...
      while (parent && !parent.matches(".sidebar-toc-sidebar")) {
        if (parent.matches("li")) {
          ...
          if (parentHref) {
            const t = document.getElementById(decodeURI(parentHref).slice(1));
            if (t) {
